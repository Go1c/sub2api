...
- `backend/internal/server/routes/user.go`（keys/groups 路由须接受该 token）
...
- 若是 user access token（前缀或 hash 查库）→ 校验：
  ...
  - 未过期
  - 用户 active
- 写入与现有一致的 `AuthSubject{UserID,...}`，使现有 `/keys` handler 的 `subject.UserID` 继续生效
...
- （若前端/客户端创建 key 需要）`GET /api/v1/groups/rates`

**拒绝**（示例，非穷尽）：

- `/api/v1/user/profile`、password、webhook、totp
- `/api/v1/usage*`、payment、subscriptions、admin、redeem 等
...
现有 keys API 已按 `subject.UserID` 校验所有权；本卡需测试确保：
...
- [x] 有效 access token 访问 profile/usage/admin 等 → 403
