...
浮窗欢迎语始终取后台「AI 客服 → 欢迎语」，留空时使用当前语言的系统默认文案，不使用 gateway 返回的欢迎语。读取站点公开设置后，先应用自定义文案和人工联系入口，再请求外部 gateway，因此网关可用与否都不影响欢迎语。标题与联系信息仍沿用站点设置优先的合并规则。

回归测试覆盖网关配置成功或失败时的自定义文案和人工联系链接，以及未配置文案时忽略网关欢迎语、使用系统默认文案。
