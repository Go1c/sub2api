...
**`lottery_campaigns`**：`id`、`name varchar(120)`、`subtitle varchar(240)`、`status varchar(20)`（`active` / `finished`）、`prize_count`、`max_participants`、`joined_count default 0`、`winner_count default 0`、`created_by`、`created_at`、`updated_at`、`finished_at null`。索引：`status`、`created_at desc`。应用层不变量：只有一个活动可为 `active`，服务通过在创建新活跃活动前把现有活跃活动置 `finished` 来强制。
...
5. 计算 `remainingPrizes = prize_count - winner_count`、`remainingSlots = max_participants - joined_count`、`winProbability = remainingPrizes / remainingSlots`。
6. 随机决定结果。
...
**站内信发放**：使用 `SiteMessageService.SystemSendToUser` 或等价内部方法——绕过普通用户每日发送上限、校验站内信启用、以管理员或系统发送者创建消息、返回 `site_message_id`。消息文案：标题 `恭喜中奖：{活动名}`；正文 `你在「{活动名}」中中奖。` / `兑换码：{code}` / `请复制该兑换码前往兑换页面使用。`。若消息创建失败，draw 事务回滚——绝不在没有可发放码消息的情况下记录中奖者。
...
- `POST /api/v1/admin/lottery/campaigns` — 创建。校验：`name` 必填且 ≤120 runes；`subtitle` 默认 `登录就有机会，转一转赢取兑换码`；`prize_count >= 1`；`max_participants >= prize_count`；`len(codes) >= prize_count`；codes 去空白、非空、请求内唯一；站内信必须启用。
...
- **对话框**：`LotteryDialog.vue` 保留转盘动画；中奖结果面板不渲染码，显示 `恭喜你中奖！兑换码已通过站内信发放，请前往站内信领取。`，并提供打开 `/site-messages` 的按钮；未中文案保持简短中性。
- **管理员页**：`LotteryView.vue` 调管理员 API 创建活动、从后端加载历史、显示活跃 / 结束状态、参与人数和中奖者、详情面板显示未分配码和中奖记录；站内信禁用时禁用创建并显示清晰错误。
...
- 中奖码只走站内信，结果弹窗不展示码。
- 不在本次范围：加权奖品、多活跃活动、公开免登录抽奖、抽奖奖品的邮件发放、自动兑换、跨设备持久化关闭 / dismiss 状态。
