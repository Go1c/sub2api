...
description: 号池自动巡检：定时扫描请求健康条，成功率过低则加入指定分组、去掉指定模型并关闭 429 豁免，401 停调度可通知 Telegram
metadata:
  type: doc
  level: L2
  status: 已实现
...
账号列表「自动巡检」把一键巡检做成后台配置：按间隔扫描最近请求健康条，账号成功率低于阈值且每条有流量的出口都失败时，自动并入指定分组、去掉指定模型，并可同时关闭账号的 429 限流豁免。登录 401 导致停止调度时，可额外通知 Telegram 机器人。
...
- **关闭 429 豁免**：`close_429_exemption_on_degrade`（默认开）。降级 OpenAI OAuth 系账号时同时写 `extra.oauth429_cooldown_enforced=true`（repo `UpdateExtra` key 级合并，走 scheduler outbox），让真实不健康的账号重新按上游 Retry-After 冷却；单独开启也能作为唯一降级动作。详见 [[openai-oauth-429-exemption]]。
...
- 不自动恢复：成功率回来后不会自行移出分组、加回模型，也不会重新打开 429 豁免。
- 不改 IP 组选路和健康条写路径。
...
- 429 豁免：[[openai-oauth-429-exemption]]
