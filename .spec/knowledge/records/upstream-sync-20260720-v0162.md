...
metadata:
  type: record
  date: 2026-07-20
  status: #234–#249 已合入 dev；大块 B/C/E 仍评估跳过默认，F 不合；运维默认不动
---
...
| 代号 | 上游 | 用户决定 | 评估结论 |
|---|---|---|---|
| A | #4553 WS turn lifecycle | **合**（用户确认必要） | ✅ PR #249 已合：turn lifecycle + lifecycle 测试；**保留 fork CloseNow-first** 租约释放，测试断言服务端 close error。 |
...
| D | #4496+#4547 调度冷却 | **合** | ✅ PR #247 已合：池模式 temp 规则 + 模型级 `SetModelRateLimit` 隔离。 |
...
| PR | 内容 | 状态 |
|---|---|---|
| #247 | #4496+#4547 调度冷却 | ✅ 已合 |
| #248 | 大块评估文档 | ✅ 已合 |
| #249 | #4553 WS turn lifecycle | ✅ 已合（fork CloseNow 适配） |

仍跳过 / 未做：B #4590 Trae 缓存、C 受保护视频、E client-ip 全栈、F branding SVG；生产 PG 不上机；docker-deploy raw 不动。
