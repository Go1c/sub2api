...
func TestMigration924CreatesGroupUsageRollups(t *testing.T) {
	content, err := FS.ReadFile("924_group_usage_daily_rollups.sql")
...
func TestMigration924InvalidatesClosedBucketsWhenUsageLogsChange(t *testing.T) {
	content, err := FS.ReadFile("924_group_usage_daily_rollups.sql")
	require.NoError(t, err)
...
func TestMigration925TracksConfiguredTimezone(t *testing.T) {
	content, err := FS.ReadFile("925_group_usage_rollup_timezone.sql")
	require.NoError(t, err)
