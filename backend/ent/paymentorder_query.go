...
func (_q *PaymentOrderQuery) QuerySubscriptionCreditLedgers() *SubscriptionCreditLedgerQuery {
...
func (_q *PaymentOrderQuery) WithSubscriptionCreditLedgers(opts ...func(*SubscriptionCreditLedgerQuery)) *PaymentOrderQuery {
	query := (&SubscriptionCreditLedgerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	...
	if query := _q.withSubscriptionCreditLedgers; query != nil {
		if err := _q.loadSubscriptionCreditLedgers(ctx, query, nodes,
			func(n *PaymentOrder) { n.Edges.SubscriptionCreditLedgers = []*SubscriptionCreditLedger{} },
			func(n *PaymentOrder, e *SubscriptionCreditLedger) {
...
func (_q *PaymentOrderQuery) loadSubscriptionCreditLedgers(ctx context.Context, query *SubscriptionCreditLedgerQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *SubscriptionCreditLedger)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptioncreditledger.FieldOrderID)
	}
	query.Where(predicate.SubscriptionCreditLedger(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.SubscriptionCreditLedgersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
