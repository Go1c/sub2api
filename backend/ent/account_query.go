...
type AccountQuery struct {
	...
	withErrorHistories *AccountErrorHistoryQuery
...
// QueryErrorHistories chains the current query on the "error_histories" edge.
func (_q *AccountQuery) QueryErrorHistories() *AccountErrorHistoryQuery {
	query := (&AccountErrorHistoryClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withErrorHistories: _q.withErrorHistories.Clone(),
...
// WithErrorHistories tells the query-builder to eager-load the nodes that are connected to
// the "error_histories" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AccountQuery) WithErrorHistories(opts ...func(*AccountErrorHistoryQuery)) *AccountQuery {
	query := (&AccountErrorHistoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withErrorHistories = query
	return _q
}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withErrorHistories; query != nil {
		if err := _q.loadErrorHistories(ctx, query, nodes,
			func(n *Account) { n.Edges.ErrorHistories = []*AccountErrorHistory{} },
			func(n *Account, e *AccountErrorHistory) { n.Edges.ErrorHistories = append(n.Edges.ErrorHistories, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadErrorHistories(ctx context.Context, query *AccountErrorHistoryQuery, nodes []*Account, init func(*Account), assign func(*Account, *AccountErrorHistory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(accounterrorhistory.FieldAccountID)
	}
	query.Where(predicate.AccountErrorHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.ErrorHistoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
