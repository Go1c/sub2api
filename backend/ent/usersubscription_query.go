...
type UserSubscriptionQuery struct {
	...
	withCreditLedger   *SubscriptionCreditLedgerQuery
...
// QueryCreditLedger chains the current query on the "credit_ledger" edge.
func (_q *UserSubscriptionQuery) QueryCreditLedger() *SubscriptionCreditLedgerQuery {
...
func (_q *UserSubscriptionQuery) Clone() *UserSubscriptionQuery {
	...
	return &UserSubscriptionQuery{
		...
		withCreditLedger:   _q.withCreditLedger.Clone(),
...
// WithCreditLedger tells the query-builder to eager-load the nodes that are connected to
// the "credit_ledger" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserSubscriptionQuery) WithCreditLedger(opts ...func(*SubscriptionCreditLedgerQuery)) *UserSubscriptionQuery {
	query := (&SubscriptionCreditLedgerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCreditLedger = query
	return _q
}
...
func (_q *UserSubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserSubscription, error) {
	...
	if query := _q.withCreditLedger; query != nil {
		if err := _q.loadCreditLedger(ctx, query, nodes,
			func(n *UserSubscription) { n.Edges.CreditLedger = []*SubscriptionCreditLedger{} },
			func(n *UserSubscription, e *SubscriptionCreditLedger) {
				n.Edges.CreditLedger = append(n.Edges.CreditLedger, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserSubscriptionQuery) loadCreditLedger(ctx context.Context, query *SubscriptionCreditLedgerQuery, nodes []*UserSubscription, init func(*UserSubscription), assign func(*UserSubscription, *SubscriptionCreditLedger)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*UserSubscription)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptioncreditledger.FieldSubscriptionID)
	}
	query.Where(predicate.SubscriptionCreditLedger(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usersubscription.CreditLedgerColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
