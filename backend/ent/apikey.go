...
type APIKey struct {
	...
	// Fallback API key used when this key's group has no usable upstream account
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldID, apikey.FieldUserID, apikey.FieldGroupID, apikey.FieldFallbackKeyID:
...
func (_m *APIKey) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case apikey.FieldFallbackKeyID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field fallback_key_id", values[i])
			} else if value.Valid {
				_m.FallbackKeyID = new(int64)
				*_m.FallbackKeyID = value.Int64
			}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	if v := _m.FallbackKeyID; v != nil {
		builder.WriteString("fallback_key_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
