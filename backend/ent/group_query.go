...
func (_q *GroupQuery) QuerySubscriptionCreditLedgers() *SubscriptionCreditLedgerQuery {
...
func (_q *GroupQuery) WithSubscriptionCreditLedgers(opts ...func(*SubscriptionCreditLedgerQuery)) *GroupQuery {
	query := (&SubscriptionCreditLedgerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withSubscriptionCreditLedgers; query != nil {
		if err := _q.loadSubscriptionCreditLedgers(ctx, query, nodes,
			func(n *Group) { n.Edges.SubscriptionCreditLedgers = []*SubscriptionCreditLedger{} },
			func(n *Group, e *SubscriptionCreditLedger) {
...
func (_q *GroupQuery) loadSubscriptionCreditLedgers(ctx context.Context, query *SubscriptionCreditLedgerQuery, nodes []*Group, init func(*Group), assign func(*Group, *SubscriptionCreditLedger)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptioncreditledger.FieldGroupID)
	}
	query.Where(predicate.SubscriptionCreditLedger(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.SubscriptionCreditLedgersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
