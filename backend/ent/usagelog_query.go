...
func (_q *UsageLogQuery) QuerySubscriptionCreditLedgers() *SubscriptionCreditLedgerQuery {
...
func (_q *UsageLogQuery) WithSubscriptionCreditLedgers(opts ...func(*SubscriptionCreditLedgerQuery)) *UsageLogQuery {
	query := (&SubscriptionCreditLedgerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UsageLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UsageLog, error) {
	...
	if query := _q.withSubscriptionCreditLedgers; query != nil {
		if err := _q.loadSubscriptionCreditLedgers(ctx, query, nodes,
			func(n *UsageLog) { n.Edges.SubscriptionCreditLedgers = []*SubscriptionCreditLedger{} },
			func(n *UsageLog, e *SubscriptionCreditLedger) {
...
func (_q *UsageLogQuery) loadSubscriptionCreditLedgers(ctx context.Context, query *SubscriptionCreditLedgerQuery, nodes []*UsageLog, init func(*UsageLog), assign func(*UsageLog, *SubscriptionCreditLedger)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptioncreditledger.FieldUsageLogID)
	}
	query.Where(predicate.SubscriptionCreditLedger(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usagelog.SubscriptionCreditLedgersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
