...
func (_q *APIKeyQuery) QuerySubscriptionCreditLedgers() *SubscriptionCreditLedgerQuery {
...
func (_q *APIKeyQuery) WithSubscriptionCreditLedgers(opts ...func(*SubscriptionCreditLedgerQuery)) *APIKeyQuery {
	query := (&SubscriptionCreditLedgerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withSubscriptionCreditLedgers; query != nil {
		if err := _q.loadSubscriptionCreditLedgers(ctx, query, nodes,
			func(n *APIKey) { n.Edges.SubscriptionCreditLedgers = []*SubscriptionCreditLedger{} },
			func(n *APIKey, e *SubscriptionCreditLedger) {
...
func (_q *APIKeyQuery) loadSubscriptionCreditLedgers(ctx context.Context, query *SubscriptionCreditLedgerQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *SubscriptionCreditLedger)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptioncreditledger.FieldAPIKeyID)
	}
	query.Where(predicate.SubscriptionCreditLedger(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.SubscriptionCreditLedgersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
