...
func TestUserRateLimitConstants(t *testing.T) {
	...
	require.Equal(t, 10, userUsageExportRPM)
...
func TestUserUsageExportRateLimitFailCloseWhenRedisUnavailable(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	r.GET("/usage/export", limiter.LimitWithOptions("user-usage-export", userUsageExportRPM, time.Minute, appmiddleware.RateLimitOptions{
		KeyFunc: func(c *gin.Context) string {
			if subject, ok := servermiddleware.GetAuthSubjectFromContext(c); ok && subject.UserID > 0 {
				return "user:7"
			}
