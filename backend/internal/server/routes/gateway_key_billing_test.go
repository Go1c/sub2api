...
// UpdateLastUsed is a no-op: auth middleware always TouchLastUsed after successful key auth.
...
func newKeyBillingRouteTestRouter(runMode string) (*gin.Engine, *keyBillingRouteRateRepo, string) {
	...
	gatewayHandler := handler.NewGatewayHandler(
		gatewayService, nil, nil, nil, nil, nil, nil, apiKeyService,
		nil, nil, nil, nil, cfg, nil,
	)
	gatewayHandler.SetOpenAIGatewayService(openAIGatewayService)
