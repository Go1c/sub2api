...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	builder := r.client.APIKey.Create().
		...
		SetNillableFallbackKeyID(key.FallbackKeyID).
	...
	m, err := r.activeQuery().
		...
		WithUser(func(q *dbent.UserQuery) {
			q.Select(
				...
				user.FieldWebsocketNotifyEnabled,
				user.FieldWebsocketBalanceAlertEnabled,
				user.FieldWebsocketBalanceAlertThreshold,
				user.FieldWebsocketSiteMessageNotifyEnabled,
				user.FieldWebsocketAnnouncementNotifyEnabled,
				user.FieldWebhookBalanceNotifyEnabled,
				user.FieldWebhookBalanceNotifyURL,
				user.FieldWebhookBalanceNotifyThreshold,
		...
		WithGroup(func(q *dbent.GroupQuery) {
			q.Select(
				...
				group.FieldFallbackGroupIDOnExhausted,
...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey) error {
	...
	// Fallback key (nil 清空)
	if key.FallbackKeyID != nil {
		builder.SetFallbackKeyID(*key.FallbackKeyID)
	} else {
		builder.ClearFallbackKeyID()
	}
...
func userEntityToService(u *dbent.User) *service.User {
	...
	out := &service.User{
		...
		WebsocketNotifyEnabled:             u.WebsocketNotifyEnabled,
		WebsocketBalanceAlertEnabled:       u.WebsocketBalanceAlertEnabled,
		WebsocketBalanceAlertThreshold:     u.WebsocketBalanceAlertThreshold,
		WebsocketSiteMessageNotifyEnabled:  u.WebsocketSiteMessageNotifyEnabled,
		WebsocketAnnouncementNotifyEnabled: u.WebsocketAnnouncementNotifyEnabled,
		WebhookBalanceNotifyEnabled:        u.WebhookBalanceNotifyEnabled,
		WebhookBalanceNotifyURL:            u.WebhookBalanceNotifyURL,
		WebhookBalanceNotifyThreshold:      u.WebhookBalanceNotifyThreshold,
