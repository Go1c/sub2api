...
func opsFilterIncludesRecoveredProviderRows(filter *service.OpsErrorLogFilter, phaseFilter string) bool {
	// phase=upstream is the dedicated provider-health view: always include recovered
	// (<400) upstream rows. Other provider phases still require an explicit opt-in.
	...
	if phaseFilter != "" {
		return phaseFilter == "account_auth"
	}
	if len(filter.ErrorPhasesAny) == 0 {
		return false
	}
