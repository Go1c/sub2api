...
func TestPrepareUsageLogInsert_PersistsCorrelationID(t *testing.T) {
	...
	require.Equal(t, sql.NullString{String: corr, Valid: true}, prepared.args[len(prepared.args)-4])
}
...
func TestPrepareUsageLogInsert_PersistsImageSizeMetadata(t *testing.T) {
	...
	// Indices must match usageLogInsertArgTypes / prepareUsageLogInsert args order.
	// image_size lives after user_agent + ip_address + image_count.
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			0.0, 0.8, // subscription_cost_usd, balance_cost_usd
			1.0,
			sql.NullFloat64{},
