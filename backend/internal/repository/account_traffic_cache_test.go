...
func TestAccountTrafficKeysUseKinNamespaceWithoutLeases(t *testing.T) {
	require.Equal(t, []string{"kin:acct_tc:{42}:state", "kin:acct_tc:{42}:minute"}, trafficKeys(42))
	server, cache, plan := trafficFixture(t)
	_, err := cache.Acquire(context.Background(), plan, "one")
	require.NoError(t, err)
	require.True(t, server.Exists("kin:acct_tc:{42}:state"))
	require.True(t, server.Exists("kin:acct_tc:{42}:minute"))
	require.False(t, server.Exists("kin:acct_tc:{42}:leases"))
	require.False(t, server.Exists("account_traffic:{42}:state"))
}
...
func TestAccountTrafficAdaptiveSuggestionAndSlowRecovery(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		require.NoError(t, err)
		require.True(t, a.Allowed)
		require.Equal(t, 4, a.State.EffectiveConcurrency)
	}
	// Adaptive automatic concurrency is enforced by outer ConcurrencyCache,
	// not Lua acquire. A fifth in-flight request is still admitted here.
	fifth, err := cache.Acquire(ctx, plan, "fifth")
	require.NoError(t, err)
	require.True(t, fifth.Allowed)
	require.Equal(t, 4, fifth.State.EffectiveConcurrency)
	for i := 0; i < 3; i++ {
...
func TestAccountTrafficRefreshExpiresStateHash(t *testing.T) {
	_, cache, plan := trafficFixture(t)
	ctx := context.Background()
	_, err := cache.Acquire(ctx, plan, "lease")
	require.NoError(t, err)
	ok, err := cache.Refresh(ctx, plan.AccountID, "lease")
	require.NoError(t, err)
	require.True(t, ok)
	ok, err = cache.Refresh(ctx, 99, "missing")
	require.NoError(t, err)
	require.False(t, ok)
}

type trafficInFlightStub struct{ n int }

func (s trafficInFlightStub) GetAccountConcurrency(context.Context, int64) (int, error) {
	return s.n, nil
}

func TestAccountTrafficSnapshotInFlightUsesConcurrencyCache(t *testing.T) {
	_, cache, plan := trafficFixture(t)
	cache.concurrency = trafficInFlightStub{n: 7}
	ctx := context.Background()
	_, err := cache.Acquire(ctx, plan, "one")
	require.NoError(t, err)
	state, err := cache.Snapshot(ctx, plan)
	require.NoError(t, err)
	require.Equal(t, 7, state.InFlight)
	require.Equal(t, 1, state.RequestsLastMinute)
}
