		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.failure == "begin" {
				...
				query := `(?s)` + regexp.QuoteMeta("UPDATE accounts") + `.*` +
					regexp.QuoteMeta("jsonb_set(extra, '{openai_excel_bps}', 'false'::jsonb)") + `.*` +
					regexp.QuoteMeta("'{basispoints,enabled}', 'false'::jsonb") + `.*` +
					...
					regexp.QuoteMeta("extra -> 'openai_excel_bps' = 'true'::jsonb") + `.*` +
					regexp.QuoteMeta("extra -> 'basispoints' -> 'enabled' = 'true'::jsonb")
