...
type accountTrafficInFlightReader interface {
	GetAccountConcurrency(ctx context.Context, accountID int64) (int, error)
}

type accountTrafficCache struct {
	rdb         *redis.Client
	concurrency accountTrafficInFlightReader
}

func NewAccountTrafficCache(rdb *redis.Client, concurrency service.ConcurrencyCache) service.AccountTrafficCache {
	return &accountTrafficCache{rdb: rdb, concurrency: concurrency}
}

func trafficKeys(id int64) []string {
	base := fmt.Sprintf("kin:acct_tc:{%d}:", id)
	return []string{base + "state", base + "minute"}
}

// A single Lua transaction uses Redis time for the sliding 60-second ceiling
// and token-bucket burst allowance. Adaptive in-flight admission is enforced
// by Kin ConcurrencyCache, not this script. Every key for one account carries
// the same hash tag, including on Redis Cluster.
...
local minute=redis.call('ZCARD',KEYS[2])
...
if rpmOn then
 local wait=0
 if minute>=rpm then local boundary=redis.call('ZRANGE',KEYS[2],minute-rpm,minute-rpm,'WITHSCORES');wait=math.max(wait,tonumber(boundary[2])+60000-now) end
 if tokens<1 then wait=math.max(wait,(1-tokens)*60000/rpm) end
 if wait>0 then redis.call('HINCRBY',KEYS[1],'rejected_rpm',1);return {0,math.ceil(wait),1,effective,recommended,0,minute} end
 tokens=tokens-1
 redis.call('HSET',KEYS[1],'tokens',tokens)
end
redis.call('ZADD',KEYS[2],now,ARGV[10]);redis.call('EXPIRE',KEYS[2],120)
redis.call('HINCRBY',KEYS[1],'accepted',1)
return {1,0,0,effective,recommended,0,minute+1}
`)
...
func (c *accountTrafficCache) Refresh(ctx context.Context, id int64, _ string) (bool, error) {
	n, err := trafficRefreshScript.Run(ctx, c.rdb, trafficKeys(id)[:1]).Int()
	return n == 1, err
}
...
func (c *accountTrafficCache) Finish(ctx context.Context, p service.AccountTrafficPlan, id string, status int, duration int64) error {
	...
	return trafficFinishScript.Run(ctx, c.rdb, trafficKeys(p.AccountID)[:1], id, status, duration, p.Signature, enabled, p.HardLimit, v.MinConcurrency, v.FailureThreshold, v.FailureWindowSeconds, v.RecoverySeconds).Err()
}
...
func (c *accountTrafficCache) Snapshot(ctx context.Context, p service.AccountTrafficPlan) (service.AccountTrafficState, error) {
	...
	if c.concurrency != nil {
		inflight, inflightErr := c.concurrency.GetAccountConcurrency(ctx, p.AccountID)
		if inflightErr != nil {
			return state, inflightErr
		}
		state.InFlight = inflight
	}
	minute, err := c.rdb.ZCount(ctx, keys[1], "("+fmt.Sprint(now.Add(-time.Minute).UnixMilli()), "+inf").Result()
