...
const (
	...
	// UserID 认证后的 LumioAPI 用户 ID，由 API Key 认证中间件设置。
	// 供 service 层执行用户级策略，不能使用客户端请求体中的 user 标识替代。
	UserID Key = "ctx_user_id"

	// SubscriptionCandidateIDs stores usable subscription IDs that cover the
	// authenticated request group, ordered by subscription creation time.
	SubscriptionCandidateIDs Key = "ctx_subscription_candidate_ids"
