...
func responsesInputToChatMessagesWithOptions(instructions string, inputRaw json.RawMessage, opts *ResponsesToChatOptions) ([]ChatMessage, error) {
	...
	built, err := buildChatMessagesFromItems(messages, rawItems, opts)
	if err != nil {
		return nil, err
	}
...
func buildChatMessagesFromItems(messages []ChatMessage, rawItems []json.RawMessage, opts *ResponsesToChatOptions) ([]ChatMessage, error) {
	...
	for _, raw := range rawItems {
		...
		case "reasoning":
			if txt := extractResponsesReasoningText(item); txt != "" {
				...
				// No plaintext summary (encrypted-only reasoning, e.g. after
				// codex remote compaction): fall back to the gateway-side
				// cache keyed by the reasoning item id.
