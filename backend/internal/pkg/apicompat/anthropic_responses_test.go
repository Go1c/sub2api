...
func TestGPT6SolLunaKeepsSamplingWhenReasoningEffortNone(t *testing.T) {
	temperature := 0.7
	for _, model := range []string{"gpt-6-sol", "gpt-6-luna"} {
		for _, effort := range []string{"", "none", "medium", "max"} {
			out, err := ChatCompletionsToResponses(&ChatCompletionsRequest{Model: model, ReasoningEffort: effort, Temperature: &temperature, TopP: &temperature, Messages: []ChatMessage{{Role: "user", Content: json.RawMessage(`"hello"`)}}})
			require.NoError(t, err)
			if effort == "none" {
