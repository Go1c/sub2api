...
// （AccountUsageService.GetUsage），把各平台形态各异的用量数据归一成
// MonitorQuotaSnapshot，随检测历史持久化到 channel_monitor_histories.quota
// （JSONB），供管理端与用户端渲染。
//
// fork 只接 openai / anthropic / gemini / grok 的 usage 路径；Source 运行时
// 只写 "usage"。JSON 仍保留 cn_quota / cn_balance 字段定义，避免与 origin
// 快照形状不兼容，但本 fork 不实现 CNProviderQuota / CNProviderBalance 抓取。
//
...
//   - "weekly"     周窗口（JSON 兼容字段；fork 不抓 CN coding plan）
//   - "daily"      日窗口（Gemini 日配额 / Grok 日请求）
//   - "30d"        30 天窗口（Grok 月度）
//   - "total"      无窗口语义的总量额度（UsageInfo 可能附带的 per-model 条目）
//
// 同一 Window 可能出现多条（Gemini 多档日配额、Grok requests/tokens），
// 用 Label 区分：Label 是机器 token（requests/tokens/shared/pro/flash 或模型名），
...
//
// Source 取值：
//   - "usage"      海外平台（AccountUsageService.GetUsage）—— fork 唯一实现
//   - "cn_quota"   国产 Coding Plan（字段保留，fork 不抓取）
//   - "cn_balance" 国产按量付费余额（字段保留，fork 不抓取）
type MonitorQuotaSnapshot struct {
	Source    string             `json:"source"`
	Success   bool               `json:"success"`
	Tiers     []MonitorQuotaTier `json:"tiers,omitempty"`
	Balance   *float64           `json:"balance,omitempty"`    // cn_balance 主余额（JSON 兼容）
	Balances  []MonitorBalance   `json:"balances,omitempty"`   // 多币种余额（JSON 兼容）
