...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	if req.OpenAICodexClientVersion != nil {
		...
		if normalized != "" && service.NormalizeCodexClientVersion(normalized) == "" {
			response.Error(c, http.StatusBadRequest, "openai_codex_client_version must be empty or a valid version (e.g. 0.156.1)")
			return
		}
	...
	settings := &service.SystemSettings{
		...
		OpenAIImportBatchMinutes:                      stringSetting(req.OpenAIImportBatchMinutes, previousSettings.OpenAIImportBatchMinutes),
