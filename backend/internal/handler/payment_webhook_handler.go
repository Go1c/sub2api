...
// MapayNotify handles Mapay payment notifications.
// POST /api/v1/payment/webhook/mapay
func (h *PaymentWebhookHandler) MapayNotify(c *gin.Context) {
	h.handleNotify(c, payment.TypeMapay)
}
...
func extractOutTradeNo(rawBody, providerKey string) string {
	switch providerKey {
	case payment.TypeEasyPay, payment.TypeMapay, payment.TypeAlipay:
