...
func attachForkOpsRetryFields(c *gin.Context, entry *service.OpsInsertErrorLogInput) {
	if entry == nil {
		return
	}
...
func attachForkDeletedKeyAttribution(c *gin.Context, ops *service.OpsService, entry *service.OpsInsertErrorLogInput, parsedCode string) {
	if c == nil || entry == nil || parsedCode != opsCodeInvalidAPIKey {
		return
	}
	attemptedKey := extractAttemptedKey(c)
	if attemptedKey == "" {
		return
	}
	entry.AttemptedKeyPrefix = keyPrefix(attemptedKey, 8)
	if !looksLikeSystemKey(attemptedKey) || ops == nil {
		return
	}
	res, lookupErr := ops.LookupDeletedKeyAudit(c.Request.Context(), attemptedKey)
	if lookupErr != nil {
...
func OpsErrorLoggerMiddleware(ops *service.OpsService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		attachForkDeletedKeyAttribution(c, ops, entry, parsed.Code)
		attachForkOpsRetryFields(c, entry)
...
func resolveOpsPlatform(_ context.Context, apiKey *service.APIKey, fallback string) string {
