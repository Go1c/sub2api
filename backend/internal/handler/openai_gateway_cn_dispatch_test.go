...
func TestAllowOpenAICompatibleMessagesDispatch_CompositeTargetExemptsCNAndGrok(t *testing.T) {
	...
	for _, platform := range []string{service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformGrok} {
		...
		require.True(t, allowOpenAICompatibleMessagesDispatch(c, apiKey), "composite→%s 应豁免", platform)
	}
	...
	apiKey := &service.APIKey{Group: &service.Group{Platform: service.PlatformComposite, AllowMessagesDispatch: false}}
	require.False(t, allowOpenAICompatibleMessagesDispatch(c, apiKey), "composite→openai 仍受开关控制")
}
