...
func (s *BillingCacheService) SetUserPlatformQuotaRepository(repo UserPlatformQuotaRepository) *BillingCacheService {
	if s != nil {
...
// 订阅额度池与余额仍是双资金源：订阅不可用时，只要余额满足门槛就放行。
func (s *BillingCacheService) CheckBillingEligibility(ctx context.Context, user *User, apiKey *APIKey, group *Group, subscription *UserSubscription, platforms ...string) error {
	...
	isSubscriptionMode := subscription != nil && group != nil && group.IsSubscriptionType()
	// 简易模式：跳过所有计费检查
...
func (s *BillingCacheService) checkSubscriptionEligibility(ctx context.Context, userID int64, group *Group, subscription *UserSubscription) error {
	...
	if !SubscriptionCoversGroup(subscription, group, nil) {
		return ErrSubscriptionInvalid
	}
