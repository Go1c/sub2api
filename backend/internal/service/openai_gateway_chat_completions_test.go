...
func TestForwardAsChatCompletions_UnknownModelWithoutMessagesDispatchKeepsRequestedModel(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-not-a-real-model","messages":[{"role":"user","content":"hello"}],"stream":false}`)
	...
	require.Equal(t, "gpt-not-a-real-model", gjson.GetBytes(upstream.lastBody, "model").String())
...
func TestForwardAsChatCompletions_APIKeyPropagatesPromptCacheKeyInResponsesBody(t *testing.T) {
	...
	require.Equal(t, generateSessionUUID(isolateOpenAISessionID(99, "cache-key-123", account)), upstream.lastReq.Header.Get("session_id"))
}
...
func TestForwardAsChatCompletions_ResponsesShapeDoesNotAutoDerivePromptCacheKey(t *testing.T) {
	...
	require.Equal(t, generateSessionUUID(isolateOpenAISessionID(99, "explicit-responses-key", account)), upstream.requests[2].Header.Get("session_id"))
}
...
func TestGPT6ReasoningModeAndSamplingCompatibility(t *testing.T) {
	for _, model := range []string{"gpt-6-sol", "gpt-6-luna"} {
		body := []byte(`{"model":"` + model + `","reasoning":{"mode":"pro","effort":"max"},"temperature":0.7,"top_p":0.9,"top_logprobs":2,"include":["reasoning.encrypted_content","message.output_text.logprobs"]}`)
		out, changed, err := normalizeGPT6ResponsesSampling(body, model)
		require.NoError(t, err)
		...
		require.False(t, gjson.GetBytes(out, "include.1").Exists())
		require.Equal(t, "max", normalizeOpenAIReasoningEffortForModel("max", model))

		kept, keptChanged, err := normalizeGPT6ResponsesSampling([]byte(`{"model":"`+model+`","reasoning":{"effort":"none"},"temperature":0.2}`), model)
		require.NoError(t, err)
		require.False(t, keptChanged)
		require.Equal(t, 0.2, gjson.GetBytes(kept, "temperature").Float())
	}
}
