...
func TestOpenAIWSHTTPBridgeAPIKeyReusesClientToolMappingWhenFollowupOmitsTools(t *testing.T) {
	...
	account := &Account{ID: 9001, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 1}
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)

	first := []byte(`{"type":"response.create","model":"gpt-5.6-sol","stream":true,"tools":[{"type":"custom","name":"exec"}],"input":"run pwd"}`)
	_, err := svc.proxyOpenAIWSHTTPBridgeTurn(
		context.Background(), c, account, "test-token", first, len(first),
		"gpt-5", "", "", "", "", 1, func([]byte) error { return nil },
	)
	require.NoError(t, err)

	second := []byte(`{"type":"response.create","model":"gpt-5.6-sol","stream":true,"previous_response_id":"resp_custom_first","input":[{"type":"custom_tool_call_output","id":"ctco_client_output_1","call_id":"call_custom_1","output":"ok"}]}`)
	_, err = svc.proxyOpenAIWSHTTPBridgeTurn(
		context.Background(), c, account, "test-token", second, len(second),
		"gpt-5", "", "", "", "", 2, func([]byte) error { return nil },
	)
	require.NoError(t, err)
...
func TestProxyResponsesWebSocketFromClientForGrokUsesXAIHTTPBridge(t *testing.T) {
	...
	require.Equal(t, "grok-4.6", gjson.GetBytes(upstream.bodies[1], "model").String())
