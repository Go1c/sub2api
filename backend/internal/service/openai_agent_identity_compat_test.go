...
func TestOpenAIAgentIdentityPassthroughKeepsSessionAndPromptCacheHeaders(t *testing.T) {
	...
	require.Equal(t, isolateOpenAISessionID(0, "client-session", account), req.Header.Get("session_id"))
	require.Equal(t, isolateOpenAISessionID(0, "client-conversation", account), req.Header.Get("conversation_id"))
