...
func TestNonStreamingSSEToJSON_CapacityFailedEventFailsOver(t *testing.T) {
	...
	// 换号的前提：一个字节都没写出去。同账号重试只给 server_is_overloaded /
	// slow_down 这类请求级降载码（见 isOpenAIRequestScopedCapacityShed），
	// 「at capacity」走换号但不走同账号重试，与流式兄弟用例一致。
	require.False(t, failoverErr.RetryableOnSameAccount)
	require.False(t, c.Writer.Written())
	require.Empty(t, rec.Body.String())
}

func TestNonStreamingSSEToJSON_OverloadedFailedEventRetriesSameAccount(t *testing.T) {
	c, rec := newNonStreamingFailoverContext(t)
	svc := newNonStreamingFailoverService()
	body := sseTerminalBody("response.failed",
		`{"type":"response.failed","response":{"id":"resp_1","error":{"code":"server_is_overloaded","message":"Please retry later."}}}`)
