...
func TestExcelBPSSelectedModelsPreserveCodexTransportAndTickets(t *testing.T) {
	...
	for _, transport := range []OpenAIUpstreamTransport{OpenAIUpstreamTransportResponsesWebsocketV2, OpenAIUpstreamTransportResponsesWebsocketV2Ingress} {
		// main-kin transport compatibility is account-wide. A selected BPS model
		// still leaves other models on Codex, so WebSocket stays available.
		require.True(t, svc.isOpenAIAccountTransportCompatible(a, transport))
	}
	require.True(t, svc.isOpenAIAccountTransportCompatible(a, OpenAIUpstreamTransportHTTPSSE))
	require.True(t, a.IsExcelBPSEnabledForModel("gpt-6-astra"))
	require.False(t, a.IsExcelBPSEnabledForModel("gpt-6-sol"))
	require.False(t, a.isExcelBPSAllModelsEnabled())
}
...
func TestExcelBPSSelectedCompactKeepsExcelModel(t *testing.T) {
	...
	// main-kin has no Codex ticket outbound model. This fixture is passthrough,
	// so ordinary requests keep the client model while /responses/compact still
	// applies compact_model_mapping. BPS selection uses account model_mapping
	// and only matches the BPS path, which does not enter compact.
	require.Equal(t, "gpt-6-sol", resolveOpenAIAccountUpstreamModelForRequest(a, "alias", true))
	require.Equal(t, "alias", resolveOpenAIAccountUpstreamModelForRequest(a, "alias", false))
	require.True(t, a.IsExcelBPSEnabledForModel("alias"))
	require.False(t, a.IsExcelBPSEnabledForModel("gpt-6-sol"))
}
