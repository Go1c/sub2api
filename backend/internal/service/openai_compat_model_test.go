...
func TestForwardAsAnthropic_AutoDerivesPromptCacheKeyWhenMessagesDispatchHasNoSessionID(t *testing.T) {
	...
	require.Equal(t, generateSessionUUID(isolateOpenAISessionID(0, cacheKey, account)), upstream.lastReq.Header.Get("session_id"))
}
...
func TestForwardAsAnthropic_ReusesOAuthCodexTurnState(t *testing.T) {
	...
	require.Equal(t, generateSessionUUID(isolateOpenAISessionID(0, "stable-cache-key", account)), upstream.requests[1].Header.Get("session_id"))
