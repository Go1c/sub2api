...
func TestGeminiMessagesHandleStreamingResponse_ClosesToolBlockBeforeText(t *testing.T) {
	...
	for _, ev := range events {
		...
		case "content_block_start":
			require.Equalf(t, -1, open, "content block %d opened while block %d was still open", ev.index, open)
			open = ev.index
		...
		case "content_block_stop":
			require.Equalf(t, open, ev.index, "content_block_stop index %d does not match open block %d", ev.index, open)
	...
	require.True(t, textStarted)
	require.True(t, toolClosedBeforeText)
