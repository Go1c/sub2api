...
func (s *stubMonitorUsageSource) getLastAccountID() int64 {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func TestQuotaFetcher_OverseasAccountUsesUsageService(t *testing.T) {
	...
	require.Equal(t, int64(7), usage.getLastAccountID())
}

func TestQuotaFetcher_LoadsAccountOnceThenCallsGetUsage(t *testing.T) {
	fetcher, usage, accounts := newQuotaFetcherTestSetup(t)
	...
	require.Equal(t, int64(21), usage.getLastAccountID())
	require.Equal(t, 1, usage.getCalls())
}

func TestQuotaFetcher_UnsupportedPlatformYieldsErrorSnapshot(t *testing.T) {
	fetcher, usage, accounts := newQuotaFetcherTestSetup(t)
	accounts.accounts[8] = &Account{ID: 8, Platform: "kimi"}

	snapshot := fetcher.Fetch(context.Background(), 8)

	require.False(t, snapshot.Success)
	require.Contains(t, snapshot.Error, "unsupported platform")
...
func TestUsageQuotaTiers_MapsAllWindowKinds(t *testing.T) {
	...
	// 5h/7d/7d-sonnet/7d-fable + gemini×3 + grok×2 + antigravity×2
	require.Len(t, tiers, 11)
