...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedOpenAIResponsePricing(ctx, responseModel, apiKey); identified {
			...
			responseCost, responseErr := s.calculateOpenAIRecordUsageCost(
				...
				videoMultiplier, webSearchMultiplier, tokens, serviceTier, longContextBillingGate, pricingAt,
			)
	...
	usageLog.CorrelationID = usageCorrelationIDFromCtx(ctx)
	// 设置渠道信息
	...
	recordRequestHealthOK(s.requestHealth, ctx, account, input.InboundEndpoint, result.Model)
