...
func TestPassthroughLifecycle_LeaseLossSendsRetryClose(t *testing.T) {
	...
	// Fork force-closes the transport (CloseNow) so leases release promptly;
	// clients may observe EOF rather than a close frame with status/reason.
	_, err = readPassthroughLifecycleFrame(t, clientConn, 3*time.Second)
	require.Error(t, err)
	select {
	case serverSideErr := <-serverErr:
		var closeErr *OpenAIWSClientCloseError
		require.ErrorAs(t, serverSideErr, &closeErr)
