...
// EffectiveAccountTrafficConcurrency lowers the outer ConcurrencyCache cap when
// adaptive automatic is enforcing. Observe-only and read failures keep hardLimit.
func EffectiveAccountTrafficConcurrency(ctx context.Context, upstream HTTPUpstream, account *Account, hardLimit int) int {
	if hardLimit <= 0 || account == nil {
		return hardLimit
	}
	plan, err := AccountTrafficPlanFor(account)
	if err != nil || !plan.Policy.Enforces() || !plan.Policy.AdaptiveEnabled || plan.Policy.AdaptiveMode != "automatic" {
		return hardLimit
	}
	traffic := accountTrafficController(upstream)
	if traffic == nil {
		return hardLimit
	}
	state, err := traffic.State(ctx, account)
	if err != nil {
		return hardLimit
	}
	recommended := state.RecommendedConcurrency
	if recommended <= 0 {
		return hardLimit
	}
	if recommended > hardLimit {
		recommended = hardLimit
	}
	minimum := plan.Policy.MinConcurrency
	if minimum < 1 {
		minimum = 1
	}
	if minimum > hardLimit {
		minimum = hardLimit
	}
...
func (s *GatewayService) EffectiveAccountTrafficConcurrency(ctx context.Context, account *Account, hardLimit int) int {
	if s == nil {
		return hardLimit
	}
	return EffectiveAccountTrafficConcurrency(ctx, s.httpUpstream, account, hardLimit)
}

func (s *OpenAIGatewayService) EffectiveAccountTrafficConcurrency(ctx context.Context, account *Account, hardLimit int) int {
	if s == nil {
		return hardLimit
	}
	return EffectiveAccountTrafficConcurrency(ctx, s.httpUpstream, account, hardLimit)
