...
//   - openai / anthropic / gemini / grok → AccountUsageService.GetUsage
//
// fork 不接 CNProviderQuota / CNProviderBalance，也不为 antigravity 单独扩
// monitor provider；非四家平台一律产出 error 快照。
//
// Fetch 永不返回 error：所有失败都降级为 Success=false 的快照照常入库，
// 由 deriveQuotaCheckResult 推导为 failed/error 状态。
//
// 多个监控可能关联同一账号，而 interval 最小 15s，所以快照统一带 TTL 缓存
// （成功 monitorQuotaFetchCacheTTL、失败 monitorQuotaErrorCacheTTL 负缓存），
// 防止打爆上游配额端点；同账号的并发抓取由 singleflight 合并为一次上游查询。

// monitorUsageSource 海外平台账号用量查询（AccountUsageService 天然满足）。
// fork 签名是 GetUsage(ctx, accountID)，不是 origin 的 GetUsageForAccount。
...
// cfg 保留以对齐 wire 签名；fork 不读 CN 余额阈值。
...
func (f *ChannelMonitorQuotaFetcher) fetchUncached(ctx context.Context, accountID int64, now time.Time) *domain.MonitorQuotaSnapshot {
	...
	case domain.PlatformOpenAI, domain.PlatformAnthropic, domain.PlatformGemini, domain.PlatformGrok:
		return f.fetchUsage(ctx, accountID, now)
	default:
		return quotaErrorSnapshot("usage", "unsupported platform for quota fetch", now)
	}
}
...
func (f *ChannelMonitorQuotaFetcher) fetchUsage(ctx context.Context, accountID int64, now time.Time) *domain.MonitorQuotaSnapshot {
	...
	// openai/gemini/grok 的失败多走「值通道」（err==nil 但错误
...
func usageQuotaTiers(usage *UsageInfo) []domain.MonitorQuotaTier {
	...
	// UsageInfo 可能附带 per-model 总量额度（Antigravity 字段名），Label = 模型名。
	// 这不是 CN fetcher，只映射账号用量服务已有字段。
...
// usageFailureInfo 识别 GetUsage 经「值通道」返回的失败：err==nil 但把错误
// 降级在 UsageInfo 字段里（Error/ErrorCode/状态标记）。
