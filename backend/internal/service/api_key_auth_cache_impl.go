...
const apiKeyAuthSnapshotVersion = 16 // v16: group model pricing + long-context toggle
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	if apiKey.Group != nil {
		snapshot.Group = &APIKeyAuthGroupSnapshot{
			...
			WebSearchRateIndependent:        apiKey.Group.WebSearchRateIndependent,
			WebSearchRateMultiplier:         apiKey.Group.WebSearchRateMultiplier,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	if snapshot.Group != nil {
		apiKey.Group = &Group{
			...
			WebSearchRateIndependent:        snapshot.Group.WebSearchRateIndependent,
			WebSearchRateMultiplier:         snapshot.Group.WebSearchRateMultiplier,
