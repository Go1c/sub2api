...
func (s *trafficCacheStub) Snapshot(context.Context, AccountTrafficPlan) (AccountTrafficState, error) {
	return AccountTrafficState{RecommendedConcurrency: s.recommended, EffectiveConcurrency: s.recommended}, nil
}

type trafficUpstreamStub struct {
	HTTPUpstream
	traffic *AccountTrafficService
}

func (s trafficUpstreamStub) AccountTrafficController() *AccountTrafficService { return s.traffic }
...
func TestEffectiveAccountTrafficConcurrencyCapsAutomaticOnly(t *testing.T) {
	cache := &trafficCacheStub{recommended: 3}
	upstream := trafficUpstreamStub{traffic: NewAccountTrafficService(cache)}
	automatic := &Account{ID: 42, Concurrency: 8, Extra: map[string]any{AccountTrafficPolicyKey: map[string]any{"adaptive_enabled": true, "adaptive_mode": "automatic", "min_concurrency": 1}}}
	require.Equal(t, 3, EffectiveAccountTrafficConcurrency(context.Background(), upstream, automatic, 8))
	observe := &Account{ID: 42, Concurrency: 8, Extra: map[string]any{AccountTrafficPolicyKey: map[string]any{"adaptive_enabled": true, "adaptive_mode": "observe"}}}
	require.Equal(t, 8, EffectiveAccountTrafficConcurrency(context.Background(), upstream, observe, 8))
	off := &Account{ID: 42, Concurrency: 8}
	require.Equal(t, 8, EffectiveAccountTrafficConcurrency(context.Background(), upstream, off, 8))
	require.Equal(t, 8, EffectiveAccountTrafficConcurrency(context.Background(), nil, automatic, 8))
}
