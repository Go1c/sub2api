...
func TestMapGrokUpstreamStatus(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "unauthorized", status: http.StatusUnauthorized, want: http.StatusUnauthorized},
		...
		{name: "other upstream error", status: http.StatusInternalServerError, want: http.StatusBadGateway},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			require.Equal(t, tt.want, mapGrokUpstreamStatus(tt.status))
		})
	}
}
