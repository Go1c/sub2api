...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	proxyIPGroupRepository := repository.NewProxyIPGroupRepository(client, db)
	...
	openAIIPGroupBindStore := repository.NewOpenAIIPGroupBindStore(redisClient)
	openAIIPGroupResolver := service.ProvideOpenAIIPGroupResolver(proxyIPGroupRepository, proxyRepository, openAIIPGroupBindStore, concurrencyCache, configConfig)
	...
	redeemCodeRedeemer := handler.ProvideRedeemCodeRedeemer(redeemService)
	promoCodeRedeemer := handler.ProvidePromoCodeRedeemer(promoService)
	redeemHandler := handler.NewRedeemHandler(redeemCodeRedeemer, promoCodeRedeemer)
	...
	siteMessageCompensationBatchRepository := repository.NewSiteMessageCompensationBatchRepository(db)
	...
	invoiceUserReader := service.ProvideInvoiceUserReader(userRepository)
	invoiceService := service.NewInvoiceService(invoiceRepository, invoiceUserReader, emailService)
	...
	openAIOAuthService := service.ProvideOpenAIOAuthService(proxyRepository, openAIOAuthClient, privacyClientFactory, openAIIPGroupResolver)
	...
	channelMonitorQuotaFetcher := service.NewChannelMonitorQuotaFetcher(accountUsageService, accountRepository, configConfig)
	channelMonitorService := service.ProvideChannelMonitorService(channelMonitorRepository, secretEncryptor, channelMonitorQuotaFetcher)
	...
	accountErrorHistoryRepository := repository.NewAccountErrorHistoryRepository(client, db)
	accountErrorHistoryService := service.ProvideAccountErrorHistoryService(accountErrorHistoryRepository)
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, accountErrorHistoryService, grokQuotaService)
	adminAnnouncementHandler := admin.NewAnnouncementHandler(announcementService)
	adminSiteMessageHandler := admin.NewSiteMessageHandler(siteMessageService)
	...
	proxyIPGroupHandler := admin.NewProxyIPGroupHandler(adminService)
	...
	subscriptionWasteStatsRepository := repository.NewSubscriptionWasteStatsRepository(db)
	subscriptionWasteStatsService := service.NewSubscriptionWasteStatsService(subscriptionWasteStatsRepository)
	adminSubscriptionWasteStatsService := handler.ProvideSubscriptionWasteStatsService(subscriptionWasteStatsService)
	subscriptionWasteStatsHandler := admin.NewSubscriptionWasteStatsHandler(adminSubscriptionWasteStatsService)
	...
	channelIQStore := repository.NewChannelIQRepository(db)
	...
	balanceWalletRepository := repository.NewBalanceWalletRepository(db)
	balanceWalletService := service.NewBalanceWalletService(balanceWalletRepository)
	balanceClientHandler := admin.NewBalanceClientHandler(balanceWalletService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, adminSiteMessageHandler, adminLotteryHandler, adminInvoiceHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, proxyIPGroupHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, subscriptionWasteStatsHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, channelIQHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, balanceClientHandler, upstreamBillingProbeService, redisClient, adminService, gatewayService, openAIGatewayService, opsService)
	...
	userAccessTokenRepository := repository.NewUserAccessTokenRepository(client)
	userAccessTokenService := service.NewUserAccessTokenService(userAccessTokenRepository)
	userAccessTokenHandler := handler.NewUserAccessTokenHandler(userAccessTokenService)
	...
	desktopPaymentHandoffStore := repository.NewDesktopPaymentHandoffStore(redisClient)
	desktopPaymentHandoffService := service.ProvideDesktopPaymentHandoffService(desktopPaymentHandoffStore, userRepository, authService, settingService)
	desktopPaymentHandoffHandler := handler.ProvideDesktopPaymentHandoffHandler(desktopPaymentHandoffService)
	balanceWalletHandler := handler.NewBalanceWalletHandler(balanceWalletService)
	webhookBalanceNotifyService := service.ProvideWebhookBalanceNotifyService(userRepository)
	webhookBalanceNotifyWiring := service.ProvideWebhookBalanceNotifyWiring(webhookBalanceNotifyService, announcementService, siteMessageService, gatewayService, openAIGatewayService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	balanceCacheInvalidationOutboxRepository := repository.NewBalanceCacheInvalidationOutboxRepository(db)
	balanceCacheInvalidationWorker := service.ProvideBalanceCacheInvalidationWorker(balanceCacheInvalidationOutboxRepository, billingCacheService, apiKeyService)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, siteMessageHandler, lotteryHandler, invoiceHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, userAccessTokenHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelMarketHandler, batchImageHandler, asyncImageHandler, desktopPaymentHandoffHandler, balanceWalletHandler, webhookBalanceNotifyService, webhookBalanceNotifyWiring, idempotencyCoordinator, idempotencyCleanupService, balanceCacheInvalidationWorker)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService, settingService, auditLogService, userAccessTokenService)
	...
	module := server.ProvideCheckInModule(db, billingCacheService, apiKeyService)
	telemetryModule := server.ProvideTelemetryModule(db, authService)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, redisClient, module, telemetryModule)
	...
	telegramOpsSender := service.NewTelegramOpsSender()
	opsAccountErrorAlertLockStore := service.ProvideOpsAccountErrorAlertLockStore(redisClient)
	opsAccountErrorAlertService := service.ProvideOpsAccountErrorAlertService(opsService, opsRepository, telegramOpsSender, opsAccountErrorAlertLockStore, configConfig)
	...
	channelIQRunner := service.ProvideChannelIQRunner(channelIQService, leaderLockCache, db)
	...
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsAccountErrorAlertService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsUserRequestMonitorService, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, balanceCacheInvalidationWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, cnProviderBalanceCheckService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, channelIQRunner, userPlatformQuotaUsageFlusher, accountErrorHistoryWiring, auditLogService, upstreamBillingProbeService, promptService, subscriptionNotifyWorker)
...
func provideCleanup(
	...
	_ service.AccountErrorHistoryWiring,
