...
> 支付宝官方 / 微信官方、EasyPay、Mapay 可以同时作为后台服务商实例存在，但前台始终只展示 `支付宝`、`微信支付` 两个可见按钮。管理员需要分别为这两个按钮选择唯一支付来源：官方、EasyPay 或 Mapay。官方渠道直接对接 API，资金直达商户账户，手续费更低；聚合支付接入门槛更低。
...
- **支付宝**：后台启用后，需要额外指定该按钮路由到 `支付宝官方`、`易支付支付宝` 或 `码支付支付宝`
- **微信支付**：后台启用后，需要额外指定该按钮路由到 `微信官方`、`易支付微信` 或 `码支付微信`
...
Mapay 使用 `/xpay/epay/` 下的 EasyPay 风格接口。系统默认优先调用 `mapi.php` 获取二维码或支付链接；如果服务商实例选择 `弹窗` 模式，则改用 `submit.php` 托管页面。

| 参数 | 说明 | 必填 |
|------|------|------|
| **商户 ID（PID）** | Mapay 商户 ID | 是 |
| **商户密钥（PKey）** | Mapay 商户密钥 | 是 |
| **API 地址** | Mapay 站点基础地址或 `/xpay/epay` 地址，系统会自动补齐接口路径 | 是 |
| **通用通道 ID** | 所有支付方式默认使用的通道（可选） | 否 |
| **支付宝通道 ID** | 指定支付宝通道（优先于通用通道） | 否 |
| **微信通道 ID** | 指定微信通道（优先于通用通道） | 否 |

> **精确金额要求**：Mapay 会通过实际到账金额识别订单，创建订单后上游可能返回类似 `10.03` 的实付金额。前端会用醒目的动态提示展示该金额，用户必须一分不差支付；多付或少付都可能无法自动到账。
...
用户选择充值金额和支付方式
  ...
  用户完成支付
  ├─ EasyPay    → 扫码 / H5 跳转
  ├─ Mapay      → 优先内嵌二维码，必要时弹窗/跳转，并显示上游要求的精确实付金额
...
| `FAILED` | 下单或支付前失败 |
| `FULFILLMENT_FAILED` | 支付已确认，但充值/开通履约失败；系统会自动重试，仍失败后由管理员人工处理或重试 |
...
- 后台任务每 60 秒执行一次超时检查
- 支付回调验签成功后会先确认订单已支付，再执行充值/开通履约；履约失败不会向支付服务商返回 500，也不会在前端显示为“支付失败”
...
| 支付方式 | EasyPay、支付宝、微信、Stripe | 增加 Mapay（码支付） |
