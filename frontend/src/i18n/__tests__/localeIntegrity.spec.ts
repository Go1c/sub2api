...
describe('locale integrity', () => {
  ...
  it('labels check-in redeem history consistently across locales', () => {
    ...
    expect(en.redeem.balanceAddedCheckinMilestone).toBe('Check-in Milestone Day {day}')
    expect(zh.redeem.balanceAddedCheckinMilestone).toBe('签到里程碑{day}天')
    expect(zhHant.redeem.balanceAddedCheckinMilestone).toBe('簽到里程碑{day}天')
    expect(en.admin.users.typeCheckinMilestone).toBe('Balance (Check-in Milestone)')
    expect(zh.admin.users.typeCheckinMilestone).toBe('余额（签到里程碑）')
    expect(zhHant.admin.users.typeCheckinMilestone).toBe('餘額（簽到里程碑）')
  })
})
