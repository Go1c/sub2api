export default {
    accounts: {
      ...
      poolAutoInspect: {
        title: '自动巡检',
        button: '自动巡检',
        hint: '定时扫描号池最近请求健康条。当账号成功率低于阈值、且每条有流量的出口都打不进去时，自动加入指定分组并去掉指定模型。成功率恢复后不会自动移出分组或加回模型。',
        enable: '启用自动巡检',
        enableHint: '后台按设定间隔扫描当前号池，不占用列表刷新。',
        interval: '执行间隔（分钟）',
        intervalHint: '默认 5 分钟。',
        threshold: '成功率阈值（%）',
        minSamples: '最少样本数',
        addGroups: '降级后加入的分组',
        addGroupsHint: '保留原分组，再并入这里勾选的分组，例如同时留在 Codex 并加入降智分组。只会加到平台匹配的账号上。',
        removeModels: '去掉的模型',
        removeModelsHint: '从账号模型白名单中删除这些模型，例如 gpt-6-astra。',
        removeModelsPlaceholder: '输入模型 ID 后回车',
        close429Exemption: '降级时关闭 429 豁免',
        close429ExemptionHint: '降级的同时关闭账号的 429 限流豁免，让其重新按上游 Retry-After 冷却；与分组/模型降级一样，恢复后不会自动重新豁免。',
        notify401: '401 停止调度时通知机器人',
        notify401Hint: '账号因登录 401 被自动停调度时发送 Telegram。可留空 Token，改用运维异常告警里已配置的机器人。',
        cooldown: '401 通知冷却（分钟）',
        cooldownHint: '同一账号在冷却内只通知一次。',
        telegramBotToken: 'Telegram Bot Token',
        telegramChatId: 'Telegram Chat ID',
        telegramFallbackHint: '留空则复用运维设置里的账号异常 Telegram 配置。降级动作若能发到机器人，也会一并通知。',
        lastRun: '上次执行',
        runNow: '立即执行',
        running: '执行中...',
        runDone: '已触发一次巡检',
        runFailed: '立即执行失败',
        saved: '自动巡检配置已保存',
        saveFailed: '保存自动巡检配置失败',
        loadFailed: '加载自动巡检配置失败'
      },
      ...
      openai: {
        ...
        longContextBillingDesc: '默认开启。若该账号的上游不会按模型阈值收取 OpenAI API 长上下文费率，请关闭。',
        ...
        oauth429Exempt: '429 限流豁免',
        oauth429ExemptDesc: '默认开启：带 Retry-After 的瞬时 429 由同账号重试消化，不做账号级冷却停调；关闭后恢复按上游 Retry-After 冷却。号池自动巡检降级本账号时会自动关闭豁免（不自动恢复）。真实配额耗尽（5h/7d 到 100%）不受豁免影响。',
        ...
        codexFingerprintModeDesc: '多人共享同一 OAuth 账号时，将各用户的设备/会话标识收敛为账号级恒定值，减少上游可见的设备数和会话数。本分支默认「仅设备」；选「关闭」才原样透传。部分账号开启收敛后出现过额度缩水，请按自己的实测结果选择。',
      ...
      errorAlert: {
        ...
        keywords: '报错关键字',
        keywordsHint: '可多选。留空则统计该账号全部错误；填写后只统计包含这些关键字或状态码的错误。',
        keywordPlaceholder: '例如 overloaded 或 503',
        rules: '告警规则',
        rulesHint: '可选。用于单独设置时间窗口、触发次数，以及该关键字在窗口内最多推送几次。',
        noRules: '未设置规则时，使用运维后台的默认窗口、次数和冷却。',
        addRule: '添加规则',
        ruleN: '规则 {n}',
        ruleKeyword: '关键字',
        ruleKeywordPlaceholder: '留空表示该账号全部错误',
        windowMinutes: '时间窗口（分钟）',
        minErrorCount: '最少错误次数',
        maxSends: '窗口内最多推送',
        maxSendsPlaceholder: '默认 1',
        useOpsDefault: '留空使用运维默认值'
      },
      ...
      iqTestTitle: '智商检测',
      iqPromptLabel: '测试提示词',
      iqPromptPlaceholder: '例如：Generate an SVG of a pelican riding a bicycle',
      iqPromptDefault: 'Generate an SVG of a pelican riding a bicycle',
      iqPromptHint: '回复里出现完整 <svg>...</svg> 就会渲染成图片；思考过程会显示在终端里。',
      iqPreview: 'SVG 成图',
      iqExtractFailed: '未能从回复中提取 SVG',
      readyToIqTest: '准备检测。点击「开始检测」开始...',
      startIqTest: '开始检测',
      iqTesting: '检测中...',
      startingIqTestForAccount: '开始智商检测：{name}',
      sendingIqPrompt: '发送提示词...',
      audioPreview: '生成音频：',
