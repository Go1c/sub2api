export default {
  ...
  admin: {
    ...
    settings: {
      ...
      modelMarket: {
        title: '模型广场',
        description: '配置前台 /models 页面展示哪些模型。价格、计费模式和分组倍率实时读取当前渠道配置。',
        enabled: '启用模型广场',
        enabledHint: '关闭后前台模型广场仍可访问，但不会展示任何模型。',
        autoSync: '自动读取当前渠道',
        autoSyncHint: '开启后自动展示所有可公开访问的活跃渠道模型；关闭后可手动勾选展示范围和排序。',
        displayTitle: '页面标题',
        displayTitlePlaceholder: '模型广场',
        displayDescription: '页面说明',
        displayDescriptionPlaceholder: '按平台、分组和计费类型查看当前可用模型。',
        readOnlyPricingHint: '这里不配置价格。模型价格读取渠道定价，分组倍率读取分组配置；私有/专属分组不会进入公开模型广场。',
        candidateCount: '候选模型',
        visibleCount: '当前展示',
        currentMode: '同步模式',
        autoSyncMode: '自动同步',
        manualMode: '手动选择',
        autoPreview: '自动同步预览',
        autoPreviewHint: '以下模型来自当前活跃渠道和公开分组，保存后会按实时渠道信息展示。',
        manualSelection: '手动选择模型',
        manualSelectionHint: '勾选要展示的模型并设置排序；价格和倍率仍从渠道与分组实时读取。',
        show: '展示',
        sortOrder: '排序',
        platform: '平台',
        model: '模型',
        ...
        noPricing: '未配置价格',
        empty: '暂无可公开展示的模型，请先配置活跃渠道、模型定价和公开分组。',
        customModels: '自定义模型',
        customModelsHint: '用于补充渠道自动读取不到的模型；价格在这里填写，分组倍率可按选中分组单独覆盖。',
