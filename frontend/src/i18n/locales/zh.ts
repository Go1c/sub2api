export default {
  ...
  profile: {
    ...
    accessTokens: {
      title: 'API 访问令牌',
      description:
        '创建可撤销的长效令牌，供外部程序管理你自己的 API 密钥（创建/更新/删除密钥、查看可用分组），并只读查询使用日志、钱包余额与订阅额度。默认 7 天，最长 30 天。请妥善保管，勿用于他人资源。',
      name: '名称',
      namePlaceholder: '例如：CI 密钥管理',
      nameRequired: '请填写名称',
      expiresInDays: '有效天数',
      expiresHint: '默认 7 天，范围 1–30 天',
      invalidDays: '有效天数须为 1–30 的整数',
      create: '创建令牌',
      createSuccess: '令牌已创建',
      createFailed: '创建失败',
      onceWarning: '请立即复制完整令牌。离开本页后将无法再次查看。',
      copy: '复制',
      copied: '已复制',
      copyFailed: '复制失败',
      dismissOnce: '我已保存，关闭提示',
      empty: '暂无访问令牌',
      prefix: '前缀',
      status: '状态',
      statusLabels: {
      ...
      createdAt: '创建时间',
      revoke: '撤销',
      revokeConfirm: '确定撤销令牌「{name}」？撤销后立即失效。',
      revokeSuccess: '已撤销',
      revokeFailed: '撤销失败',
      loadFailed: '加载令牌列表失败'
    },
  ...
  admin: {
    ...
    channelMonitor: {
      ...
      form: {
        ...
        checkModeIQ: '状态 + 智商',
        checkModeIQHint: '先打算术探活记服务器，再打糖果题记智商，两次请求分开',
        iqQuestion: '测试题',
        iqQuestionHint: '智商检测单独发送这道题，不占用探活请求；答案判定可开模糊匹配',
        iqAnswer: '标准答案',
        iqFuzzyMatch: '模糊匹配答案',
        linkedAccount: '关联账号',
    ...
    accounts: {
      ...
      noIpGroup: '不使用 IP 组',
      ipGroupHint: '选组后出站只换代理，同一条对话钉在第一次选中的 IP。',
      ipGroupConcurrency: '每 IP 并发 {count}',
      concurrency: '并发数',
      loadFactor: '负载因子',
    ...
    proxies: {
      ...
      ipGroupMembers: '成员代理',
      ipGroupCreate: '新建 IP 组',
      ipGroupEdit: '编辑 IP 组',
      ipGroupDelete: '删除 IP 组',
      ipGroupDeleteConfirm: "确定要删除 IP 组 '{name}' 吗？",
      ipGroupSaved: 'IP 组已保存',
      ipGroupDeleted: 'IP 组已删除',
      ipGroupEmpty: '还没有 IP 组',
      ipGroupInUse: '仍有账号使用该 IP 组，请先解绑',
      editProxy: '编辑代理',
      deleteProxy: '删除代理',
      ad: {
        inline: '正在寻找合适的代理 IP？'
    ...
    settings: {
      ...
      gatewayForwarding: {
        ...
        grokCrossClientMapHint: '默认开启，便于 Codex / Claude 客户端打到 Grok 分组。关闭后不再把其他厂商模型名静默改写到 Grok。',
        ...
        claudeOAuthSystemPromptBlocksPlaceholder: '留空时使用内置 3 个 blocks。支持 JSON 数组，或带 blocks 字段的 JSON 对象。',
      ...
      modelMarket: {
        ...
        readOnlyPricingHint: '自动读取模型默认按 Token 当前价格展示；也可以在候选模型中手动填写 Token 价格，或切换为按次、按次图片、按次视频并填写展示单价。图片计费支持 1K / 2K / 4K 分档，视频计费支持 480P / 720P / 1080P 分档，未填写的分档不会在前台展示。后台表单没有缓存读取/创建输入框；已手填过的覆盖用「恢复自动价格」整条清掉，只清空输入/输出清不掉缓存覆盖。',
        candidateCount: '候选模型',
        visibleCount: '当前展示',
        currentMode: '同步模式',
        autoSyncMode: '自动同步',
    ...
    lottery: {
      ...
      create: {
        title: '新建抽奖',
        description: '设置奖品数量和兑换码，发布后用户登录即可弹窗抽奖。奖品数不少于参与名额时全员中奖。',
        name: '活动名称',
        ...
        promoSection: '关注引导（可选）',
        promoSectionHint: '同一套文案和图片会出现在抽奖前、中奖结果和中奖站内信。图片必须是备份 S3 / R2 的公开 HTTPS 直链，不能用预签名附件链接。',
        promoText: '引导文案',
        promoTextPlaceholder: '关注公众号领取更多活动',
        promoImageUrl: '海报图片 HTTPS 链接',
        promoImageUrlPlaceholder: 'https://pub-xxxx.r2.dev/lottery/wechat-qr.png',
        promoImageUrlHint: '把图片上传到现有备份桶（建议 key：lottery/wechat-qr.png），再粘贴公开访问域名下的直链。不要用 S3 API 地址或带附件下载头的预签名 URL。',
        promoPreviewEmpty: '填写 HTTPS 链接后可在此预览',
        guaranteedWinHint: '奖品数不少于参与名额时，每位参与者都会中奖（例如 100 人 100 份 = 100% 中奖）。',
        wheelNote: '用户看到的转盘固定为 8 格，与奖品数量无关。',
        codes: '兑换码',
        ...
        previewPromoText: '关注微信公众号，领取更多兑换码与活动通知',
        saved: '✓ 已发布，可切换到「历史抽奖记录」查看',
        invalid: '请检查必填项和兑换码数量',
        invalidPromoImage: '海报链接必须是 https:// 公开直链',
        failed: '创建抽奖活动失败'
      },
      history: {
        title: '历史抽奖记录',
  ...
  payment: {
    ...
    admin: {
          ...
          syncPlanLimitsPreviewMessage: '将当前套餐日限 {dailyLimit}、周限 {weeklyLimit} 同步到已购订阅。匹配有效订阅 {matched} 个，其中 {changed} 个会更新；不影响总额度、有效期、覆盖范围或已用量。',
  ...
  invoice: {
    ...
    history: '开票历史',
    submit: '申请',
    fillLastSuccess: '一键填写',
    fillLastSuccessDone: '已填入上次开票成功的抬头、税号和邮箱',
    noCompletedHistory: '暂无开票成功的历史记录可填写',
    download: '下载',
