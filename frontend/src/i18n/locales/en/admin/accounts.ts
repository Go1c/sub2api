export default {
    accounts: {
      ...
      poolAutoInspect: {
        title: 'Auto inspect',
        button: 'Auto inspect',
        hint: 'Periodically scan the pool request-health bars. When an account success rate is below the threshold and every sampled IP is failing, join the selected groups and remove the selected models. Recovery does not automatically undo those changes.',
        enable: 'Enable auto inspect',
        enableHint: 'A background job scans the account pool on the interval below. It does not slow list refresh.',
        interval: 'Interval (minutes)',
        intervalHint: 'Default is 5 minutes.',
        threshold: 'Success-rate threshold (%)',
        minSamples: 'Minimum samples',
        addGroups: 'Groups to join on degrade',
        addGroupsHint: 'Keep current groups and also join the ones selected here, e.g. Codex plus a fallback group. Only matching platforms are bound.',
        ...
        cooldownHint: 'The same account is notified at most once per cooldown.',
        telegramBotToken: 'Telegram Bot Token',
        telegramChatId: 'Telegram Chat ID',
        telegramFallbackHint: 'If empty, reuse the Telegram bot from ops account error alerts. Degrade actions are also sent when a bot is available.',
        lastRun: 'Last run',
        runNow: 'Run now',
        running: 'Running...',
        runDone: 'Inspection cycle started',
        runFailed: 'Failed to run inspection',
        saved: 'Auto inspect settings saved',
        saveFailed: 'Failed to save auto inspect settings',
        loadFailed: 'Failed to load auto inspect settings'
      },
      ...
      openai: {
        ...
        longContextBillingDesc:
          'Enabled by default. Turn this off when the account\'s upstream does not charge OpenAI API long-context rates above the model threshold.',
        ...
        codexFingerprintModeDesc: 'When multiple users share the same OAuth account, converge device/session identifiers to account-level stable values to reduce upstream-visible device and session count. This branch defaults to Device only; choose Off to pass client identifiers through. Some accounts reported quota shrinkage after enabling convergence, so choose based on your own measurements.',
      ...
      errorAlert: {
        ...
        keywordsHint: 'Multi-select. Leave empty to count all errors for this account; otherwise only errors containing these keywords or status codes are counted.',
        keywordPlaceholder: 'e.g. overloaded or 503',
        rules: 'Alert rules',
        rulesHint: 'Optional. Override the window, trigger count, and how many times this keyword can be sent in the window.',
        noRules: 'Without rules, the ops-settings window, count, and cooldown are used.',
        addRule: 'Add rule',
        ruleN: 'Rule {n}',
        ruleKeyword: 'Keyword',
        ruleKeywordPlaceholder: 'Leave empty to match all errors for this account',
        windowMinutes: 'Window (minutes)',
        minErrorCount: 'Minimum error count',
        maxSends: 'Max sends in window',
        maxSendsPlaceholder: 'Default 1',
        useOpsDefault: 'Leave empty to use ops defaults'
      },
      ...
      iqTestTitle: 'IQ Test',
      iqPromptLabel: 'Test prompt',
      iqPromptPlaceholder: 'Example: Generate an SVG of a pelican riding a bicycle',
      iqPromptDefault: 'Generate an SVG of a pelican riding a bicycle',
      iqPromptHint: 'A complete <svg>...</svg> in the reply is rendered as an image. Thinking text stays in the terminal.',
      iqPreview: 'SVG preview',
      iqExtractFailed: 'Could not extract an SVG from the reply',
      readyToIqTest: 'Ready to test. Click "Start IQ Test" to begin...',
      startIqTest: 'Start IQ Test',
      iqTesting: 'Testing...',
      startingIqTestForAccount: 'Starting IQ test for account: {name}',
      sendingIqPrompt: 'Sending prompt...',
