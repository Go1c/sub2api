  ...
  <BaseDialog :show="show" :title="title" width="full" :close-on-click-outside="true" @close="close">
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div v-if="detail.deleted_key_owner_user_id" class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          ...
          <div class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
            ...
            <span class="ml-1 text-gray-500 dark:text-gray-400">#{{ detail.deleted_key_owner_user_id }}</span>
            <span v-if="detail.deleted_key_name" class="ml-2 font-mono text-xs text-gray-500 dark:text-gray-400">{{ detail.deleted_key_name }}</span>
          </div>
        </div>
...
type DiagnosticPayloadKey = 'client' | 'upstream_message' | 'upstream_detail' | 'upstream_events' | 'request_body'
...
const diagnosticPayloadSections = computed(() => {
  ...
  const candidates: Array<{ key: DiagnosticPayloadKey; value: string }> = [
    ...
    { key: 'request_body', value: meaningfulPayload(current.request_body) }
  ]
