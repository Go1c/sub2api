        ...
        <div v-if="order" class="rounded-xl bg-white p-5 shadow-sm dark:bg-dark-800">
          <div class="space-y-3 text-sm">
            ...
            <div class="flex justify-between">
              ...
              <span class="font-medium text-gray-900 dark:text-white">{{ baseAmount.toFixed(2) }} {{ t('payment.creditUnit') }}</span>
            </div>
            <div v-if="order.fee_rate > 0" class="flex justify-between">
              ...
              <span class="font-medium text-gray-900 dark:text-white">{{ feeAmount.toFixed(2) }} {{ t('payment.creditUnit') }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</span>
              <span class="font-bold text-primary-600 dark:text-primary-400">{{ order.pay_amount.toFixed(2) }} {{ t('payment.creditUnit') }}</span>
            </div>
        ...
        <div v-else-if="returnInfo" class="rounded-xl bg-white p-5 shadow-sm dark:bg-dark-800">
          <div class="space-y-3 text-sm">
            ...
            <div v-if="returnInfo.money" class="flex justify-between">
              <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</span>
              <span class="font-medium text-gray-900 dark:text-white">{{ returnInfo.money }} {{ t('payment.creditUnit') }}</span>
            </div>
...
const isFulfillmentFailed = computed(() => {
  return normalizeOrderStatus(order.value?.status) === 'FULFILLMENT_FAILED'
})
...
const statusTitle = computed(() => {
  ...
  if (isFulfillmentFailed.value) {
    return t('payment.result.fulfillmentPending')
  }
