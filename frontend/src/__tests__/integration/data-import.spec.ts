...
describe('ImportDataModal', () => {
  ...
  it('applies OpenAI JSON file import defaults to the actual data endpoint', async () => {
    ...
    expect(account.proxy_ip_group_id).toBeUndefined() // Server resolves dynamic/group/single priority.
    expect(account).toMatchObject({ concurrency: 8, group_ids: [5], extra: { keep: true, codex_fingerprint_mode: 'device', error_alert: { enabled: false }, turn_state_probe: { enabled: true }, openai_oauth_responses_websockets_v2_mode: 'passthrough', openai_oauth_responses_websockets_v2_enabled: true, account_traffic_control: { strict_rpm_enabled: true, adaptive_enabled: true, rpm: 60, burst: 5, adaptive_mode: 'observe' } } })
