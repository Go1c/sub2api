...
export async function fetchSupportChatPublicSettings(
  ...
  const payload = (await response.json()) as
    | SupportChatPublicSettings
    | APIEnvelope<SupportChatPublicSettings>
...
function inferEventType(event: SupportChatEvent) {
  ...
  if (event.conversation_id || event.conversationId || event.id) return 'id'
  if (event.error) return 'error'
  return 'end'
}

function normalizeSources(
  input: SupportChatSource | SupportChatSource[] | undefined
): SupportChatSource[] {
  if (!input) return []
