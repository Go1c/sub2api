...
function defaultPaymentMode(providerKey: string): string {
  if (providerKey === 'easypay' || providerKey === 'mapay') return PAYMENT_MODE_QRCODE
  return ''
}
...
function providerSupportsPaymentMode(providerKey: string): boolean {
  // Fork retains mapay gateway mode selection (qrcode/popup).
  return providerKey === 'easypay' || providerKey === 'alipay' || providerKey === 'mapay'
}
...
function isValidPaymentMode(providerKey: string, mode: string): boolean {
  if (providerKey === 'easypay' || providerKey === 'mapay') {
