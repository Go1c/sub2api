...
describe('PaymentMethodSelector', () => {
  ...
  it('keeps built-in alipay brand styling for exact alipay method keys', () => {
    const wrapper = mount(PaymentMethodSelector, {
      props: {
        selected: 'alipay',
  ...
  it('keeps balance method selected styling', () => {
    const wrapper = mount(PaymentMethodSelector, {
      props: {
        selected: 'balance',
        methods: [{ type: 'balance', fee_rate: 0, available: true }],
      },
    })

    const button = wrapper.get('button')
    expect(button.classes()).toContain('border-emerald-500')
  })
})
