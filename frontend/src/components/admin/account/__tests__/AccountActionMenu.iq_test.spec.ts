...
describe('AccountActionMenu — IQ test visibility', () => {
  it('shows IQ test for OpenAI OAuth accounts', () => {
    const wrapper = mount(AccountActionMenu, {
      props: { show: true, account: makeAccount({ platform: 'openai', type: 'oauth' }), anchorRect },
  ...
  it('shows IQ test for OpenAI API key accounts', () => {
    const wrapper = mount(AccountActionMenu, {
      props: { show: true, account: makeAccount({ platform: 'openai', type: 'apikey' }), anchorRect },
      attachTo: document.body
    })
    expect(getBodyText()).toContain('admin.accounts.iqTest')
    wrapper.unmount()
  ...
  it('hides IQ test for Claude accounts', () => {
    const wrapper = mount(AccountActionMenu, {
      props: { show: true, account: makeAccount({ platform: 'anthropic', type: 'apikey' }), anchorRect },
      attachTo: document.body
    })
