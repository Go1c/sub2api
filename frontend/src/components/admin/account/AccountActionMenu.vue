      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="isOpenAIAccount" @click="$emit('iq-test', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="brain" size="sm" class="text-fuchsia-500" :stroke-width="2" />
              {{ t('admin.accounts.iqTest') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'iq-test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow'])
const { t } = useI18n()
