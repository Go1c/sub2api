      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button @click="$emit('error-history', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="exclamationTriangle" size="sm" class="text-red-500" />
              {{ t('admin.accounts.errorHistory.menu') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'error-history', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy'])
const { t } = useI18n()
